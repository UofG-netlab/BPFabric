...
{
    uint32_t limit = MIN(node->prefixlen, key->prefixlen);
    uint32_t prefixlen = 0, i = 0;
    ...
    if (trie->data_size >= 8)
    {
        uint64_t diff = be64toh(*(__be64 *)node->data ^
                                *(__be64 *)key->data);
    ...
    {
        uint32_t diff = be32toh(*(__be32 *)&node->data[i] ^
                                *(__be32 *)&key->data[i]);
    ...
    {
        uint16_t diff = be16toh(*(__be16 *)&node->data[i] ^
                                *(__be16 *)&key->data[i]);
...
void *trie_lookup_elem(struct bpf_map *map, void *_key)
{
...
/* Called from syscall or from eBPF program */
int trie_update_elem(struct bpf_map *map,
...
/* Called from syscall or from eBPF program */
int trie_delete_elem(struct bpf_map *map, void *_key)
{
    struct lpm_trie *trie = container_of(map, struct lpm_trie, map);
    struct bpf_lpm_trie_key *key = _key;
...
#define LPM_VAL_SIZE_MAX ((1UL << 11) - LPM_DATA_SIZE_MAX - \
...
int trie_get_next_key(struct bpf_map *map, void *_key, void *_next_key)
{
    ...
    node_stack = malloc(trie->max_prefixlen * sizeof(struct lpm_trie_node *));
    if (!node_stack)
        return -ENOMEM;
...
static uint64_t trie_mem_usage(const struct bpf_map *map)
{
    struct lpm_trie *trie = container_of(map, struct lpm_trie, map);
    uint64_t elem_size;
