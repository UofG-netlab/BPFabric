...
int ubpf_load_elf(struct ubpf_vm *vm, const void *elf, size_t elf_size, char **errmsg)
{
    ...
    for (i = 0; i < ehdr->e_shnum; i++)
    {
        ...
        for (j = 0; j < rel->size / sizeof(Elf64_Rel); j++)
        {
            ...
            if (ELF64_R_TYPE(r->r_info) == 1 && sym->st_shndx == maps_idx)
            {
            ...
            else if (ELF64_R_TYPE(r->r_info) == 1)
            {
                ...
                if (insns[insn_idx].opcode != (EBPF_CLS_LD | EBPF_SRC_IMM | EBPF_SIZE_DW)) // LDDW instruction
                {
                    *errmsg = ubpf_error("bad relocation for instruction 0x%x at index %d\n", insns[insn_idx].opcode, insn_idx);
                    goto error;
                }
